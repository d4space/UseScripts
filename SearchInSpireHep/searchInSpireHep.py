...
def GetDate(item):
    ...
    if 'imprint' in item.keys():
        ...
        response = requests.get('http://doi.org/'+GetDOI(item))
        if response.status_code != 200: print('getting the paper infor. failed')
        #response.encoding = 'utf-8'
        date=re.search(r'First Online: </span><span class="article-dates__first-online"><time datetime="([0-9]{4}-[0-9]{2}-[0-9]{2})">',response.text).group(1)
        #date=re.search(r'First Online: </span><span class="article-dates__first-online"><time datetime="([0-9]{4}-[0-9]{2}-[0-9]{2})">',response.content).group(1)
    else:
